...
func Run(reservoirs []Reservoir) {
	...
	for r := range reservoirs {
		for i := range reservoirs[r].ExpellerItem.IngesterItems {
			...
			statsChans["queues"][queueName] = make(chan string)
			clearChans["queues"] = make(map[string]chan struct{})
			clearChans["queues"][queueName] = make(chan struct{})
			...
			statsChans["ingesters"][ingesterName] = make(chan string)
			clearChans["ingesters"] = make(map[string]chan struct{})
			clearChans["ingesters"][ingesterName] = make(chan struct{})
			...
			for d := range reservoirs[r].ExpellerItem.IngesterItems[i].DigesterItems {
				...
				statsChans["digesters"][digesterName] = make(chan string)
				clearChans["digesters"] = make(map[string]chan struct{})
				clearChans["digesters"][digesterName] = make(chan struct{})
		...
		statsChans["expellers"][expellerName] = make(chan string)
		clearChans["expellers"] = make(map[string]chan struct{})
		clearChans["expellers"][expellerName] = make(chan struct{})
